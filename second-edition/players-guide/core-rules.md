...
- 豁免是为了避免危险选择带来负面结果而进行的掷骰。角色掷 1d20，并将结果与相应属性比较。结果等于或低于该属性则成功，否则失败。掷出 1 总是成功，掷出 20 总是失败。
- 如果两个对手都试图压制对方，由风险更高的一方进行豁免。
- 如果两个角色需要共同完成一个行动，由风险更高的一方进行豁免（通常是相关属性最低的角色）。

## 治疗与恢复

- 休息片刻并喝点水可以恢复损失的 HP，但也可能让队伍暴露在危险之中。绷带可以稳定受到重创的角色。
...
角色在自己的回合中可以移动最多 40 英尺，并进行至多一个行动。行动可以是施放法术、攻击、再次移动，或其他合理的行为。每回合，PC 都要在掷骰前声明自己要做什么。如果角色尝试危险的事情，守望者会要求合适的玩家或 NPC 进行豁免。
