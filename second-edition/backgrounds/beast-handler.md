---
title: 驯兽师
layout: default
parent: 角色过往
grand_parent: 第二版
---

# 驯兽师

> 只有你能无所畏惧地走入野兽之间，并掌控它们。你与动物之间的联系，是其他人只能梦想拥有的……只要你不会变成它们的食物。

## 姓名

阿玛拉、伍尔夫、米蕾耶、索伦、弗雷基、阿斯特、格里克、博里阿斯、维达、马修斯

## 初始装备

- 3d6 金币
- 口粮（3 次）
- 火把（3 次）
- 皮鞭（d6）
- 催眠飞镖（力量豁免，否则陷入睡眠，6 次）
- 诱饵
- 绳子（25 英尺）

## 你最擅长与哪类动物打交道？掷 1d6：

|       |               |                                                                                                                                                                                   |
| ----- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | **蛛形纲动物** | 获得一根**速燃棒**和一个**油罐**（6 次）。它能在几秒内摧毁一个大型蜘蛛巢。                                                                            |
| **2** | **猫科动物**   | 获得一袋**须草**。它的气味甚至能安抚并控制最大的猫科动物。                                                                                           |
| **3** | **犬科动物**   | 获得一串**狼毒草**和一张**大网**。对狼人同样有效。                                                                                         |
| **4** | **鸟类**       | 获得一枚**鸟鸣哨**（3 点充能）。它能模仿任何鸟叫，甚至可以用来传递简单消息。**充能**：像鸟妈妈一样喂一只雏鸟，然后吹响哨子。 |
| **5** | **啮齿动物**   | 获得一支**排箫**，能发出只有啮齿动物听得见的高频声音。只要你持续吹奏，它们就会跟随你，哪怕走向死亡。                                     |
| **6** | **蛇类**       | 获得一块**温热石**，能产生无法抗拒的热量，以及一小瓶**解毒剂**（2 次）。                                                                               |


## 你从野兽那里学到了什么？掷 1d6：

|       |                                                                                                                                                                                          |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | 世界远比表面看起来广阔。只要安静地集中精神，你就能借用附近一只**你熟悉的动物**的感官。                                       |
| **2** | 野兽的行为本身就是一种语言。观察你熟悉的**动物**时，你可以洞察天气模式和即将发生的灾害。                             |
| **3** | 狩猎的脉搏是一种强大的冲动。你能感知捕食者——即使你**并不熟悉它们**——是否在附近。                                                        |
| **4** | 大地本身就是一种语言。在你熟悉的动物栖息的地形中，你迷路的概率降低一级（例如从 4-in-6 降为 3-in-6）。             |
| **5** | 只要跟上自然的节奏，你就能听见它的交响乐。当你被熟悉的动物包围时，它们会在危险到来之前提醒你。               |
| **6** | 生存全靠适应。每天一次，你可以获得一种你熟悉的动物所拥有的简单特征（例如蹼状手指、夜视能力等）。每次这样做都增加一点**疲劳**。 |
